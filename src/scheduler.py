...
def schedule_pending_jobs():
    ...
    for job in pending:
        ...
        if abs(job.run_at-now)<timedelta(minutes=5) and job.status==JobStatus.PENDING:
            print("Schedule,",abs(job.run_at-now))
            
            redis_client.zadd(zset_key, {f"job:{job.id}": now.timestamp()+job.estimated_runtime})
            job_store.update_status(job.id,JobStatus.SCHEDULED)
...
def update_next_run(job_id):
    ...
    job_store.update_run_at(job_id,next)
